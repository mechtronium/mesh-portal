...
use mesh_portal_versions::version::v0_0_1::wave::{AsyncRequestHandler, ReqCtx, ReqShell, RespCore, RespShell, Wave};
...
impl Drivers {
    ...
    pub fn init(&self) -> Result<(),MsgErr> {
        ...
        for driver in self.drivers.values() {
            if driver.status() != DriverStatus::Ready && driver.status() != DriverStatus::Initializing {
...
impl TraversalLayer for Drivers {
    ...
    async fn towards_core_router(&self, traversal: Traversal<Wave>) {
        match self.drivers.get( &traversal.record.details.stub.kind ) {
            None => {
                traversal.logger.warn( format!("star does not have a driver for Kind <{}>", traversal.record.details.stub.kind.to_string()) );
            }
            Some(driver) => {
...
impl From<StarSkel> for DriverSkel {
    fn from(skel: StarSkel) -> Self {
        Self {
            ...
            towards_fabric_router: skel.towards_fabric_router,
