...
{
    let cert = rcgen::generate_simple_self_signed(vec!["hyperlane".to_string()])?;
...
impl HyperServerQuic {
   ...
   pub fn close(self)  {
       self.endpoint.close(VarInt::from_u64(0u64).unwrap(), "no reason given".as_bytes())
   }
}
...
impl HyperClientQuic {
    pub async fn new(endpoint: Endpoint,
        ...
        let new_connection = endpoint.connect(server_addr, "hyperlane")?.await?;
